...
- Adding a topic = new `topics/<slug>/index.html` (copy an existing one) **and** a `.card` entry in the correct section of `index.html` **and** a link in every page's `.ribbon-nav` (there is no template — the ribbon is repeated per page, so all six pages change together).
