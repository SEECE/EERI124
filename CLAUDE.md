...
- **Topic pages** ([topics/](topics/)) — one folder per topic, each a self-contained `index.html`. All three are **stepwise solvers** sharing one script (`js/solver-page.js`): `simple-resistive-circuits` (resistors + voltage sources → KCL, KVL, equivalent resistance), `current-sources` (adds the independent current source → KCL, KVL, with the known-current / supermesh / constraint steps live) and `dependent-sources` (adds the four controlled sources → KCL, KVL, with step 7's constraint equations live). See [structure/SOLVER.md](structure/SOLVER.md).
