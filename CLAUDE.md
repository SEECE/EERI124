...
- **Topic pages** ([topics/](topics/)) — one folder per topic, each a self-contained `index.html`, and they come in **three kinds**:
  ...
  - **Tutorial pages** — the two §3 deep dives `wheatstone-bridge` and `delta-wye`: interactive guides, **not** problem sets. No generator, no technique, no stepper, no File menu — a hand-drawn figure, dials that drive it, and a guide walked chapter by chapter that re-renders against the live numbers. Two regions (board + lesson) instead of three. The bridge page also lets you hide an arm and null the bridge to measure it. `js/tutorial/{draw,lesson,delta-wye,wheatstone}.js` over `css/tutorial.css`. See [structure/TUTORIALS.md](structure/TUTORIALS.md).
...
- **CSS** ([css/](css/)) — split by scope, ≤200 lines each, loaded in order. `tokens.css` +
  ...
  and none of the solver ones; home adds `home.css`.
